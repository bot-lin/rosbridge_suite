...
class BasicNavigator:
    ...
    def __init__(self, node_handle):
        ...
        self.node_handle.get_logger().info("222222222222222222222222")
        ...
        self.feedback = None
        self.status = None
        self.node_handle.get_logger().info("333333")
        ...
        self.nav_through_poses_client = ActionClient(self.node_handle,
                                                     NavigateThroughPoses,
                                                     'navigate_through_poses')
        self.nav_to_pose_client = ActionClient(self.node_handle, NavigateToPose, 'navigate_to_pose')
        self.follow_waypoints_client = ActionClient(self.node_handle, FollowWaypoints, 'follow_waypoints')
        self.follow_path_client = ActionClient(self.node_handle, FollowPath, 'follow_path')
        self.compute_path_to_pose_client = ActionClient(self.node_handle, ComputePathToPose,
                                                        'compute_path_to_pose')
        self.compute_path_through_poses_client = ActionClient(self.node_handle, ComputePathThroughPoses,
                                                              'compute_path_through_poses')
        self.spin_client = ActionClient(self.node_handle, Spin, 'spin')
        self.backup_client = ActionClient(self.node_handle, BackUp, 'backup')
        self.node_handle.get_logger().info("444444")
        self.localization_pose_sub = self.node_handle.create_subscription(Pose,
                                                              'robot_pose',
                                                              self._amclPoseCallback,
                                                              amcl_pose_qos)
        self.node_handle.get_logger().info("555555")
        self.initial_pose_pub = self.node_handle.create_publisher(PoseWithCovarianceStamped,
                                                      'initialpose',
                                                      10)
        self.change_maps_srv = self.node_handle.create_client(LoadMap, '/map_server/load_map')
        self.clear_costmap_global_srv = self.node_handle.create_client(
            ClearEntireCostmap, '/global_costmap/clear_entirely_global_costmap')
        self.clear_costmap_local_srv = self.node_handle.create_client(
            ClearEntireCostmap, '/local_costmap/clear_entirely_local_costmap')
        self.get_costmap_global_srv = self.node_handle.create_client(GetCostmap, '/global_costmap/get_costmap')
        self.get_costmap_local_srv = self.node_handle.create_client(GetCostmap, '/local_costmap/get_costmap')
    ...
    def lifecycleStartup(self):
        ...
        for srv_name, srv_type in self.get_service_names_and_types():
            if srv_type[0] == 'nav2_msgs/srv/ManageLifecycleNodes':
                ...
                mgr_client = self.node_handle.create_client(ManageLifecycleNodes, srv_name)
    ...
    def _waitForNodeToActivate(self, node_name):
        ...
        state_client = self.node_handle.create_client(GetState, node_service)
        self.node_handle.get_logger().info("6666666")

        while not state_client.wait_for_service(timeout_sec=1.0):
            self.info(f'{node_service} service not available, waiting...')
        self.node_handle.get_logger().info("7777777")
        
        req = GetState.Request()
        state = 'unknown'
        self.node_handle.get_logger().info("8888888")
        ...
        while state != 'active':
            ...
            time.sleep(2)
        self.node_handle.get_logger().info("99999")
    ...
    def info(self, msg):
        self.node_handle.get_logger().info(msg)
        return

    def warn(self, msg):
        self.node_handle.get_logger().warn(msg)
        return

    def error(self, msg):
        self.node_handle.get_logger().error(msg)
        return

    def debug(self, msg):
        self.node_handle.get_logger().debug(msg)
        return